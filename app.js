...
// ====================================================== // STORAGE KEYS // ======================================================
const STORAGE_KEYS = { user: "flexHubUser", userToken: "flexHubUserToken", vipToken: "flexHubVipToken" };
// ====================================================== // GLOBAL VARIABLES // ======================================================
let currentUser = null; let allPredictions = [];
let currentLeagueFilter = "all"; let currentResultFilter = "all"; let currentSearchQuery = "";
// ====================================================== // PAGE INITIALIZATION // ======================================================
...
function getMatchKickoffDate(prediction) { if (!prediction?.match_date || !prediction?.match_time) { return null; }
...
function formatMatchCountdown(milliseconds) { const totalMinutes = Math.max( 0, Math.floor(milliseconds / 60000) );
...
return ${minutes}M; }
function updateMatchCountdowns() { document .querySelectorAll(".match-countdown") .forEach((element) => { const date = element.dataset.matchDate; const time = element.dataset.matchTime;
        if (!date || !time) {
            return;
        }
...
function createPredictionCard(prediction) { const kickoffTime = getMatchKickoffDate(prediction); const status = String( prediction.status || "pending" ).toLowerCase();
const category =
    String(
        prediction.category ||
        "regular"
...
<span class="team-badge" data-team-name="${escapeHtml( prediction.home_team || "Home Team" )}" style=" width:36px; height:36px; min-width:36px; border-radius:50%; display:flex; align-items:center; justify-content:center; background:#111722; border:2px solid #f5b942; overflow:hidden; "
${escapeHtml(
...
<span class="team-badge" data-team-name="${escapeHtml( prediction.away_team || "Away Team" )}" style=" width:36px; height:36px; min-width:36px; min-height:36px; border-radius:50%; display:flex; align-items:center; justify-content:center; background:#111722; border:2px solid #f5b942; overflow:hidden; "
${escapeHtml(
...
${escapeHtml( prediction.away_team || "Away Team" )} 
...
MATCH STARTS IN: ${kickoffTime ? formatMatchCountdown( kickoffTime.getTime() - Date.now() ) : "—" } 
            <strong>
                ${escapeHtml(
                    prediction.prediction ||
                    "Preview"
...
// ====================================================== // MESSAGE DISPLAY // ======================================================
function showElementMessage( element, message, type = "info" ) {
if (!element) {
    return;
}
...
(function setupForgotPassword() { function startForgotPassword() { const existing = document.getElementById("forgotPasswordOverlay"); if (existing) existing.remove();
...
function connectForgotPasswordButton() { const button = document.getElementById("forgotPasswordButton");
...
if (document.readyState === "loading") { document.addEventListener( "DOMContentLoaded", connectForgotPasswordButton ); } else { connectForgotPasswordButton(); }
// The login gate can be rendered dynamically, so keep checking // briefly until the button exists. let attempts = 0;
const finder = setInterval(() => { attempts++;
if (connectForgotPasswordButton() || attempts >= 30) {
  clearInterval(finder);
}
}, 500); })();
// ===================================================== // END OF FORGOT PASSWORD // =====================================================
// ====================================================== // ============================================================ // BETTING CODES // ============================================================ // ============================================================ // BETTING CODES // ============================================================
...
// ============================================================ // END BETTING CODES // ============================================================ // ============================================================ // ============================================================ // VIP BETTING CODES // ============================================================
...
// ============================================================ // END VIP BETTING CODES // ============================================================ // REAL TEAM BADGES // ============================================================
...
// Watch prediction cards for team badges const teamBadgeObserver = new MutationObserver(() => {
loadTeamBadges();
});
teamBadgeObserver.observe(document.body, { childList: true, subtree: true });
// Load badges already on the page setTimeout(() => { loadTeamBadges(); }, 1000);
// ============================================================ // END REAL TEAM BADGES // ============================================================ // ============================================================ // FLEX HUB LOGIN / REGISTER ANIMATION // ============================================================
